<script lang="ts">
	...
	onMount(() => {
		// Extract theme color as reliable RGB by rendering a pixel
		try {
			...
			if (tctx) {
				...
				tctx.fillStyle = computedColor;
				tctx.fillRect(0, 0, 1, 1);
				const px = tctx.getImageData(0, 0, 1, 1).data;
				themeColor = `rgb(${px[0]}, ${px[1]}, ${px[2]})`;
			}
		} catch (e) {
	...
	async function generatePoster() {
		...
		try {
			...
			const [qrImg, coverImg, avatarImg] = await Promise.all([
				...
				avatar ? loadImage(avatar).catch(() => null) : Promise.resolve(null),
			]);
